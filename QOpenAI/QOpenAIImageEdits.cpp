...
void QOpenAIImageEdits::sendRequest(const QString &prompt) {
    ...
    assert(QFileInfo::exists(_image));
    assert(QFileInfo::exists(_mask));
    ...
    imagePart.setHeader(QNetworkRequest::ContentDispositionHeader, "form-data; name=\"image\"; filename=\"selected_image_copy.png\"");
    qDebug() << "Image ->" << _image;
    imagePart.setBody(_image.toUtf8());
    QHttpPart maskPart;
    maskPart.setHeader(QNetworkRequest::ContentTypeHeader, "image/png");
    maskPart.setHeader(QNetworkRequest::ContentDispositionHeader, "form-data; name=\"mask\"; filename=\"selected_image_mask.png\"");
    qDebug() << "Mask ->" << _mask;
